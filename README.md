...
- Production-сайт работает в режиме `public`; доступ и изоляцию обеспечивает
  собственная авторизация CarPulse. Первый администратор уже создан, поэтому
  регистрация с телефона открыта без риска перехвата bootstrap-роли.
...
- bot profile и список команд `/start`, `/stop` настраиваются вместе с webhook;
...
8. Сайт public, поэтому production-деплой требует явного подтверждения
   пользователя и `deploy_site_version`.
...
## Rollout аккаунтов и Telegram

- первый администратор создан;
- Sites access mode переключён в `public`;
- Telegram webhook активен на `/api/telegram/webhook`;
- у бота настроены описание и команды `/start`, `/stop`;
- каждый пользователь создаёт собственный одноразовый код в настройках;
- webhook принимает только запросы с секретным Telegram header.
...
1. Проверить регистрацию и вход со второго телефона.
2. Проверить, что два пользователя не видят радары/избранное друг друга.
3. Подключить Telegram к двум тестовым аккаунтам и отправить test notification.
4. Поднять VPS collector и убрать зависимость от localhost.
5. Проверить 2–3 автоматических цикла production и Telegram delivery.
6. Добавить email verification, self-service password reset и admin audit log.
